...
export function readDat(buffer) {
  ...
  // Timing plans: 10 plans × 8 entries × 20 bytes
  for (let p = 0; p < 10; p++) {
    const entries = [];
    for (let e = 0; e < 8; e++) {
  ...
  for (let p = 0; p < data.dayPlans.length; p++) {
    ...
    for (const entry of plan) {
      ...
      for (const sp of entry.splits) {
        ph.push(sp[0]); // 단일값만 (Python 파서와 동일: 중복쌍의 첫번째)
      }
      periods.push({
        plan: p,
...
export function detectManufacturer(buffer) {
  ...
  // Empty manufacturer string → 한진이엔씨 (한진 DAT files have zeros at 0x395A)
  if (!name.trim()) {
    return '한진이엔씨';
  }
