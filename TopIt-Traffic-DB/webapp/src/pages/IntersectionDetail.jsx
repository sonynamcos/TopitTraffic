...
import { getIntersection, getFileUrl, updateIntersection } from '../api/client';
...
export default function IntersectionDetail() {
  ...
  return (
    <div>
      <Link to="/list" className="back-link">← 목록으로</Link>
      ...
      <div className="detail-grid">
        ...
        <div className="card">
          ...
          <div className="btn-group">
            {dat?.filename && (
              <a href={getFileUrl(id, 'dat')} className="btn" download>
                DAT 다운로드
              </a>
            )}
            {cycle?.filename && (
              <a href={getFileUrl(id, 'cycle')} className="btn" download>
                주기표 다운로드
              </a>
            )}
          </div>
        ...
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <table className="plan-table">
            ...
            <tbody>
              {dat.plans.map(plan => (
                <tr key={plan.plan}>
                  <td style={{ fontWeight: 600 }}>Plan {plan.plan}</td>
                  <td>{plan.cycle}</td>
                  <td>{plan.offset}</td>
                  {plan.splits.map((s, i) => (
                    <td key={i}>
