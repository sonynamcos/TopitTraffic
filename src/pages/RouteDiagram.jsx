...
export default function RouteDiagram() {
  ...
  function getSearchResults() {
    ...
    return graph.nodes
      .filter(n => n.name.toLowerCase().includes(q) || String(n.id).toLowerCase().includes(q))
      .slice(0, 8);
  }
            ...
            {selEdge !== null && selectedEdge && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 6 }}>
                  선: {getNode(selectedEdge.from)?.name} &mdash; {getNode(selectedEdge.to)?.name}
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
